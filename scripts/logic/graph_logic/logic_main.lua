...
super_mario_bros_wonder_location = {}
super_mario_bros_wonder_location.__index = super_mario_bros_wonder_location
...
function super_mario_bros_wonder_location.new(name)
    local self = setmetatable({}, super_mario_bros_wonder_location)
    if name then
...
function super_mario_bros_wonder_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = super_mario_bros_wonder_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function super_mario_bros_wonder_location:connect_two_ways(exit, rule)
...
function super_mario_bros_wonder_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function super_mario_bros_wonder_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function super_mario_bros_wonder_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function super_mario_bros_wonder_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function super_mario_bros_wonder_location:accessibility()
...
-- 
function super_mario_bros_wonder_location:discover(accessibility, keys)
...
entry_point = super_mario_bros_wonder_location.new("entry_point")
